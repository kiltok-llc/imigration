...
export function QuizDropdown({ options, ...props }: Omit<ComponentProps<typeof FormDropdown>, 'i18nKey' | 'options'> & {
...
export function QuizMultiDropdown({ options, ...props }: Omit<ComponentProps<typeof FormMultiDropdown>, 'i18nKey' | 'options'> & {
  options: { i18nKey?: string; value: string, }[]
}) {
  const rootKey = useQuizFieldKey(null);
